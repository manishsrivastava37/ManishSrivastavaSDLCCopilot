...
type Workspace = { recommendations: { recommendation: string; rationale: string; factors: string[]; analystId: string }[]; decisions: { decision: string; rationale: string; approverId: string }[]; collateral: { id: string; type: string; description: string; value: number; currency: string; status: string }[]; documents: { id: string; category: string; fileName: string; verificationStatus: string; findings?: string }[]; tasks: { taskType: string; status: string; assigneeId: string }[] };
...
function App() {
  ...
  const transition = async (application: Application, targetStatus: string, reason: string) => {
    const response = await fetch(`/api/v1/applications/${application.id}/transitions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ targetStatus, reason, actorId: targetStatus === 'Approved' ? 'approver-demo' : 'credit-demo' }) });
  ...
  const postWorkspace = async (path: string, body: unknown, absolute = false) => {
    if (!selected) return;
    const response = await fetch(absolute ? `/api/v1/${path}` : `/api/v1/applications/${selected.id}/${path}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
  ...
  return <main className="shell">
    ...
    <section className="workspace"><div className="section-heading"><div><p className="eyebrow">WORK QUEUE</p><h2>My applications</h2></div><label>Filter <select value={statusFilter} onChange={event => setStatusFilter(event.target.value)}><option value="all">All statuses</option><option>Intake</option><option>Analysis</option><option>PendingApproval</option></select></label></div>
      ...
      {visibleApplications.length > 0 && <div className="table-wrap"><table><thead><tr><th>Application</th><th>Borrower</th><th>Amount</th><th>Status</th><th>Owner</th><th>Action</th></tr></thead><tbody>{visibleApplications.map(item => <tr key={item.id}><td><button className="link-button" type="button" onClick={() => openWorkspace(item)}>{item.applicationNumber}</button></td><td>{item.borrowerLegalName}</td><td>{new Intl.NumberFormat('en-US', { style: 'currency', currency: item.currency }).format(item.requestedAmount)}</td><td><span className="status">{item.status}</span></td><td>{item.ownerId}</td><td>{item.status === 'Intake' && <button type="button" onClick={() => submitForAnalysis(item)}>Submit for analysis</button>}{item.status === 'Analysis' && <button type="button" onClick={() => transition(item, 'PendingApproval', 'Credit analysis completed')}>Submit for approval</button>}{item.status === 'PendingApproval' && <><button type="button" onClick={() => openWorkspace(item)}>Review and decide</button></>}</td></tr>)}</tbody></table></div>}
