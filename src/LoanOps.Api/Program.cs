...
app.UseExceptionHandler(exceptionApp => exceptionApp.Run(async context =>
{
    ...
    await Results.Problem("The request could not be completed.", statusCode: StatusCodes.Status500InternalServerError, title: "Unexpected server error.").ExecuteAsync(context);
}));
...
{
    if (request.TenantId != context.TenantId() || request.OwnerId != context.UserId()) return Results.Forbid();
...
{
    var application = store.Get(id);
    if (application is null || application.TenantId != context.TenantId() || request.ActorId != context.UserId()) return Results.NotFound(new { error = "Application not found." });
    ...
    return result.Error switch
    {
        ...
        "Actor and reason are required." => Results.BadRequest(new { error = result.Error }),
